...
while read -r filename; do
	...
	>"${dstdir}/${filename}_tmp"

	# avoid from generating new header files every time.
	if [ -f "${dstdir}/$filename" ] ; then
		diff -q "${dstdir}/${filename}_tmp" "${dstdir}/$filename"
		if [ $? = 0 ] ; then
			# the same
			rm -f "${dstdir}/${filename}_tmp"
			continue
		fi
	fi
	mv -f "${dstdir}/${filename}_tmp" "${dstdir}/${filename}"
done
)
