...
static int convert_range(int val, int omin, int omax, int nmin, int nmax)
{
	...
	if (orange == 0)
		return 0;
	return my_rint((((double)nrange * ((double)val - (double)omin)) + ((double)orange / 2.0)) / ((double)orange + (double)nmin));
}
#endif
...
static int convert_prange(long val, long min, long max)
{
	...
	if (range == 0)
		return 0;
	val -= min;
	tmp = my_rint((double)val/(double)range * 100);
	return tmp;
}
