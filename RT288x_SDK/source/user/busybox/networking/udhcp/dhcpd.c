...
#define BYOD_SUPPORT // define by chhung
...
int udhcpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (1) { /* loop until universe collapses */
		...
		case DHCPREQUEST:
			...
			char idx = 0, cmd[128];
			memset(cmd, 0, 128);
			parameter = get_option(&packet, DHCP_PARAM_REQ);
			if (requested && parameter) {
				sprintf(cmd, "fingerprint.sh adddb %d.%d.%d.%d ",
						*requested, *(requested+1), *(requested+2), *(requested+3));
				while (1) {
					sprintf(cmd, "%s%d", cmd, *(parameter+idx));
					if (++idx < *(parameter-1)) 
						sprintf(cmd, "%s,", cmd);
					else
						break;
