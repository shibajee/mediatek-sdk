...
static NOINLINE void display_process_list(int lines_rem, int scr_width)
{
	...
	printf(OPT_BATCH_MODE ? "%.*s" : "\033[7m%.*s\033[0m", scr_width,
		...
		USE_FEATURE_TOP_SMP_PROCESS(" CPU")
		USE_FEATURE_TOP_CPU_USAGE_PERCENTAGE(" %CPU")
		" COMMAND");
	lines_rem--;
...
# define FMT "%4u%%"
	...
	while (--lines_rem >= 0) {
		...
		col = snprintf(line_buf, scr_width,
				"\n" "%5u%6u %-8.8s %s%s" FMT
				USE_FEATURE_TOP_SMP_PROCESS(" %3d")
				USE_FEATURE_TOP_CPU_USAGE_PERCENTAGE(FMT)
				" ",
				s->pid, s->ppid, get_cached_username(s->uid),
				s->state, vsz_str_buf,
				SHOW_STAT(pmem)
				USE_FEATURE_TOP_SMP_PROCESS(, s->last_seen_on_cpu)
				USE_FEATURE_TOP_CPU_USAGE_PERCENTAGE(, SHOW_STAT(pcpu))
		);
...
int top_main(int argc UNUSED_PARAM, char **argv)
{
	...
	col = getopt32(argv, "d:n:b"USE_FEATURE_TOPMEM("m"), &str_interval, &str_iterations);
#if ENABLE_FEATURE_TOPMEM
