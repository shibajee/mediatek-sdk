...
int main( int ArgCn, const char *ArgVc[] ) {
    ...
    while (i < ArgCn) {
		...
		if (i + 1 < ArgCn && ArgVc[i+1][0] != '-') {
            ...
            case 'w':
                if (i + 1 < ArgCn && ArgVc[i+1][0] != '-') {
                    if(addRTWiFiIntf(ArgVc[i+1]) == -1)
                        log(LOG_ERR, 0, "Wrong interface name.");
                    i++;
                } else {
                    log(LOG_ERR, 0, "Missing interface name after -w option.");
                }
                break;
#endif
#endif
...
#ifdef RT3052_SUPPORT
	rt_init(snooping);		/* enable snooping */
#endif
