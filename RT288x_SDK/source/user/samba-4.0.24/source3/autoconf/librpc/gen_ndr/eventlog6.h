...
union eventlog6_EvtRpcVariantUnion {
	...
	const char *stringVal;/* [case(EvtRpcVarTypeString),unique,charset(UTF16)] */
...
struct eventlog6_EvtRpcLocalizeExportLog {
	struct {
		struct policy_handle *control;/* [ref] */
		const char *logFilePath;/* [range(1,MAX_RPC_FILE_PATH_LENGTH),charset(UTF16),ref] */
		uint32_t locale;
		uint32_t flags;
	} in;
...
struct eventlog6_EvtRpcGetPublisherList {
	...
	struct {
		...
		const char ***publisherIds;/* [range(0,MAX_RPC_PUBLISHER_COUNT),charset(UTF16),size_is(,*numPublisherIds),ref] */
		WERROR result;
	} out;
...
struct eventlog6_EvtRpcGetPublisherMetadata {
	struct {
		...
		const char *logFilePath;/* [range(0,MAX_RPC_FILE_PATH_LENGTH),unique,charset(UTF16)] */
		uint32_t locale;
		uint32_t flags;
	} in;
