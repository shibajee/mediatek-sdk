...
struct PAC_BUFFER {
	...
	union PAC_INFO *info;/* [switch_is(type),subcontext(0),flag(LIBNDR_FLAG_ALIGN8),subcontext_size(_subcontext_size_PAC_INFO(r,ndr->flags)),relative] */
...
struct PAC_BUFFER_RAW {
	enum PAC_TYPE type;
	uint32_t ndr_size;
	struct DATA_BLOB_REM *info;/* [subcontext_size(NDR_ROUND(ndr_size,8)),flag(LIBNDR_FLAG_ALIGN8),relative,subcontext(0)] */
	uint32_t _pad;/* [value(0)] */
}/* [public] */;
