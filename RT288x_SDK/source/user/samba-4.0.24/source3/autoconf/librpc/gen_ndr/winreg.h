...
struct KeySecurityData {
	uint8_t *data;/* [size_is(size),length_is(len),unique] */
	uint32_t size;
	uint32_t len;
};
...
struct winreg_EnumValue {
	struct {
		...
		uint8_t *value;/* [length_is(length?*length:0),size_is(size?*size:0),range(0,0x4000000),unique] */
...
struct winreg_QueryValue {
	struct {
		...
		enum winreg_Type *type;/* [unique] */
		uint8_t *data;/* [size_is(data_size?*data_size:0),length_is(data_length?*data_length:0),unique,range(0,0x4000000)] */
...
struct winreg_QueryMultipleValues {
	struct {
		...
		uint32_t num_values;
		uint8_t *buffer;/* [size_is(*buffer_size),length_is(*buffer_size),unique] */
